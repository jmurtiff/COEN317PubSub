...
# given the information we set, have the broker continuously listen for new connections and spawn a new thread 
def pubmasterthread():
    log(f"Publisher thread is up at port {pub_port}")

    # accept new connections, but spawn a new thread to handle that connection
    while True:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            # Accept connections and spawn new threads to handle each connection
            conn, addr = s.accept()
            threading.Thread(target=pubthread).start()

# now 
def pubthread(conn, addr):
    data = b""
    with conn:
        ...
        while True:
            ...
            if 'getProxyNodes' in convertData:
                ...
                log("SENDING PUBLISHER'S PUBLIC KEY")
...
def submasterthread():
    global sub_port, host
    log(f"Subscriber master thread is up at port {sub_port}")
    ...
    while True:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            # Accept connections and spawn new threads
            conn, addr = s.accept()
            threading.Thread(target=subthread()).start()


def subthread(conn, addr):
    data = b""
    with conn:
...
if ret_val != -1:
  ...
  try:
    threading.Thread(target=pubmasterthread).start()
    threading.Thread(target=submasterthread).start()
    threading.Thread(target=proxymasterthread).start()
